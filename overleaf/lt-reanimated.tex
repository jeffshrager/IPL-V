...
proofs more elegant than those produced by the human
authors. For Theorem~2.85, LT discovered a shorter, more direct proof
than the one published in \textit{Principia}. Simon later wrote that
they had found ``a proof of one theorem that was more elegant than the
one in Principia,'' and joked that ``Russell might have been
pleased.'' It was, arguably, the first artificial
intelligence. Simon reportedly walked into Newell's office after LT's
first overnight run and declared: ``Over Christmas, Al, we invented
thinking.''
...
tracking stacks and subgoals. The first version was famously
hand-simulated in January 1956: Simon assembled his wife and children
together with some graduate students, gave each a card representing a
component of the program, and had them execute the algorithm by
passing cards back and forth. Simon later described this as ``running
the program on the human computer''---a process that mirrors Turing's
...
free list. McCarthy's genius was not in inventing list processing
\textit{de novo}, but in finding the syntactic and runtime forms that
made these ideas---ideas that Newell, Simon, and Shaw had already
developed---elegant, accessible, and practical. Having programmed in
Lisp for fifty years and attributed its brilliance entirely to
McCarthy, building this interpreter gave me a deepened appreciation
for Newell, Simon, and Shaw's foundational contributions.
...
fluently at this point---probably also Ed Feigenbaum. I was
simultaneously learning the language myself while building the
interpreter, which meant that the LLMs' primary source of information
about IPL-V was my own Lisp code---which contained bugs.
...
teach the language itself, since unlike almost any other language they
encounter, there is no existing community or corpus for IPL-V.
...
This same problem was not lost on the original developers. In an
interview conducted by Pamela McCorduck for her pioneering history of
AI~\cite{mccorduck1979machines}---transcripts originally obtained for
the ELIZA reanimation project~\cite{lane2025eliza}---Cliff Shaw
...
J-function not implemented, a register not saved and restored, a stack
underflow. These were frustrating but tractable. A human programmer,
even working alone, could identify and fix them.

The second phase was qualitatively different. Once the interpreter was
stable enough that LT ran without crashing, the output was thousands
of lines of execution trace---machine cycles, register states, list
operations---that \textit{might} be correct. The program was
heuristic; it was supposed to explore and backtrack. Distinguishing a
correct backtrack from one caused by a subtle bug required
...
could sustain. The feeling was remarkably similar to what Shaw
described---the program would not crash, it would ``do something
reasonable,'' and I could not tell whether its behavior reflected a
bug or was simply a consequence of the heuristic search proceeding as
designed.

The LLMs, however, were not inhibited by this. They could read
thousands of lines of trace, insert diagnostic prints at ten-line
intervals, and maintain the kind of tedious but precise
...
What they are best at is tenacity: they are willing to read through
thousands of lines of traces and dumps, and insert breakpoints at
hundreds of places, until they localize the problem.

\subsection{The J74 Bug: Simon's J's to the Rescue}

After over a year of work, the interpreter had reached a state where
LT was proving trivial theorems (those requiring substitution only)
and was not crashing. However, it would not prove complex theorems
that it should have, and would get into loops, failing to recognize
when a proof had been found. By this point I had developed a strong
sense that my IPL-V interpreter basically worked---otherwise LT would
never have gotten through even a dozen instructions---and that the LT
transcription was correct. Therefore, almost all remaining errors were
likely in the J-functions, which I had written in Lisp from their
descriptions in the Newell et al.\ manual. My experience was that the
manual's J-function descriptions were sometimes vague in subtle ways,
and this had bitten me before. I was particularly wary of the most
complex J-functions: the generators (J17--J19, J100) and the list
manipulation functions (J74 and others).

Gemini found the first issue after several hours of LLM time spread
over several days, and as predicted it was in a J-function. This made
significant progress: now LT was generating proofs, but was not
stopping when a proof was found. After literally another several hours
of LLM time, Claude isolated the second problem in J74---the list
copying function. But we did not know how to fix it, because both
Claude and I agreed that my code appeared to match the manual's
description. There was a clear problem, but the specification seemed
...
IPL-V code on those cards and compare it with my Lisp
implementation of J74. The result: they differed in exactly the way we
had hypothesized. A one-line patch to my J74 implementation made LT
...
it to identify and fix a bug. It was, in a sense, a collaboration
across sixty years---Simon's original card deck, preserved by the
Computer History Museum and transcribed by its volunteers, provided
the ground truth that neither the manual's prose description, nor my
interpretation of it, nor the LLM's inference from my code, had been
able to supply.
...
recursion, higher-order functions, generators. What they lacked were
...
manipulable by Lisp programs---and automatic garbage collection.
...
LISP~1.5~\cite{millstein1968logic}. That effort, however, rewrote
LT's logic in Lisp rather than faithfully emulating IPL-V and running
the original code. The distinction is significant: a Lisp
