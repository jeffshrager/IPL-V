...
%%% 20260827 JS: Most changes are minor wording -- a diff will show them to you.
%%% I suggest rewriting all in third person, but you're right that sometimes
%%% first seems more natural. I'm sure thattThe IEEE editors will opine upon this.
%%%
...
pseudocode called the Logic Language (LL), which was simulated by hand and later called Information Processing Language One (IPL-I). LT
was revised many times. The first machine-executed version was
written in IPL-II and ran on the JOHNNIAC, and a final version 
was published in 1963 by Einar Stefferud as a pedagogical document, and written in IPL-V, the only formally released version of the language.
%% Non-seq: ", while later versions were based on empirical psychological research."
%% Also it's hard to tell what version were based on psych work and not. Simon
%% says that the original was based on how he did proofs. Maybe we could distinguish
%% empirical from formal, or something, but it's non-seq enough not to be worth it.
We describe a reanimation of both the IPL-I(LL) version of LT and of the 1963 IPL-V version. Comparing the 
two reanimations illuminates the process of the invention of early AI algorithms and computer languages.

\end{abstract}

\begin{IEEEkeywords}
Logic Theorist, IPL-I, IPL-V, artificial intelligence history,
software archaeology, A.~Newell, H.~A.~Simon, J.~C.~Shaw, list and symbol
...
%%% Is this relevant??? maybe as a nod to a possible earlier AI, but
%%% Then we should be explicit about why we're mentioning it.
...
and Russell's \textit{Principia Mathematica}~\cite{LT1957,whitehead1910principia}.\footnote{Star notion follows \textit{Principia}.}  
...
moment the project's significance was first announced: ``It was just after Christmas vacation---January
...
%%% This isn't beyond...maybe it's below ... or something, but whatever, not needed
%%% Beyond reproducing \textit{Principia} proofs, 
The Logic Theorist
used heuristic search strategies such as subgoal decomposition
\cite{LT1957}; Newell, Simon and Shaw suggested that is was a model of
human problem solving~\cite{HPS1958}, and improved the model by adding strategies
that they observed in studies of human theorem proving, such as means-ends analysis\cite{PCT1959}, methods that subsequently influenced artificial intelligence and cognitive models such as the General Problem Solver
(GPS)~\cite{newell1959gps}, SOAR~\cite{laird1991},
and ACT~\cite{anderson1993,anderson1996}.

LT co-evolved with the Information Processing Languages (LL and IPL-I through V),
a series of languages created by Newell, Shaw, and Simon  
to extend computational capabilities and understand human thought~\cite[p.~II-17]{cip16june1958}.
The IPLs introduced list manipulation~\cite[section~2.6]{knuth1997}, dynamic memory allocation~\cite{cmuarchiveshawinterview},
symbols as first-class objects, recursion, and
%%% Would need to explain, so removed  "to some extent," 
...
to 
Weizenbaum's SLIP \cite{SLIP1963} the list processing package underlying ELIZA~\cite{lane2025eliza}.  Yet while Lisp has thrived for
...
1990s~\cite{richman1995epam}.
%\footnote{Richman wrote to Shrager
...
The remainder of the present paper describes our
reanimation of two versions of the Logic Theorist:
the original 1956 version written in IPL-I~\cite{LT1956b}, and a  
later version in IPL-V\cite{stefferud1963lt}.  Following the convention
...
Our work illustrates a methodological approach that might be called
\emph{executable archaeology}: studying old computational systems by
reconstructing and running their original code. Early AIs were not
just proposals but also programs (hand-executed in the case of LT1)
which cannot be completely understood just from their descriptions.
Reanimation gives us a new look into the origins of artificial
intelligence; we return to exactly what this shows in
...
running on an emulated IBM 7094~\cite{lane2025eliza}. Although 
LT proves theorems while ELIZA attempts to converse in English,
both are part of the same list-processing lineage coming originally
from IPL.  We reanimated LT5 similarly to ELIZA, by using
an emulated IBM 7094, but for LT1 constructed a new interpreter
for the abstract IPL-I machine; we also tried this approach
with LT5.
...
symbols. They explored chess among other domains, and landed on 
logic, which Simon was exploring at the time, and seemed simpler than chess.
%% Probably should cite simon or mccorduck on this.
...
pseudocode for an abstract machine which was never implemented. The program
was tested  by human simulation~\cite[Ch.~13]{simon1996models}, 
...
from these experiments~\cite{LT1957}, and
the discovery of a more elegant proof for Theorem *2.85 than the 
one produced by Whitehead and Russell was also discovered by the program at around this 
...
analysis \cite{PCT1959}. Simon and Newell generalized their theory, eventually
creating by the General Problem Solver,
GPS~\cite[p.~19]{SHT1959}.

The second version of LT that we reanimated was converted to IPL-V by Fred Tonge, and further developed
by Einar Stefferud as a pedagogical exercise in IPL-V~\cite[Section~I]{stefferud1963lt}.
These  reanimations  bracket the program's
...
%%% Propose replacing "Human" with "Homo Sapien", as that's the "platform" :-)
...
causes a new set of working memories to be used, akin to modern register
...
In IPL-II, the first IPL that ran on a computer, the lists of elements
...
Similarly to S-expressions in Lisp, each cell contains an ordered pair of 
...
every cell can potentially be pushed and popped, and these stacks are elegantly implemented as IPL-V lists.
...
whatever is the indicated next cell in the list that represents the routine. 
% I stole "elegantly" for the above, so it seemed overused to have it here too,
% but this one speaks of its own elegance.
Because H1 is both
...
%%% I wonder if we shouldn't just write in the third person all the way
%%% through, as: "Mowes fixed..." or "DM fixed..." Then you wouldn't need this header
...
simulation~\cite[Ch.~13]{simon1996models}, \cite[p.~xx]{newell1964ipl}, this is most likely the first time this program has run on non-human hardware.
...
everything was documented in the manual.\footnote{Shrager noticed this as well in his Lisp-based IPL-V interpreter.}
To make Stefferud's code work, I had to copy a hack of  
...
%%% For example:
%%%We 
DM tried to replicate the results of \cite{LT1957}
...
%%% Maybe move to the end since JS claims it's the "most important" thing?
Most importantly, \textbf{reanimation makes historical claims checkable.} The 1956
...
and for many further experiments to be made.
...
Specifications and manuals don't cover every detail. Reamination, on the other hand
requires that such details be clarified.
...
stack. The present work takes a different, yet parallel approach, emulating 
abstract machine specifications in modern languages.
...
require substantially less effort than was required for LT.
...
I studied under both Newell and Simon at Carnegie
...
whose work this paper merely relives. 
%%% Reamimates? Recreates? "Re-runs" seems too pedestrian
